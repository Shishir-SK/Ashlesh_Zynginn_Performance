...
// Uses tokens from setup() for proper authentication
...
// Global auth data (set in setup())
let globalAuthData = null;

// Initialize with auth data from setup
export function initHelpers(authData) {
...
// HTTP request helpers with auth from setup()
export const httpHelper = {
  // Get headers with authentication from setup()
  getHeaders: (authType = 'none') => {
    if (!globalAuthData) {
      throw new Error('Helpers not initialized. Call initHelpers(authData) in setup()');
    }
    return getAuthHeaders(globalAuthData, authType);
  },
