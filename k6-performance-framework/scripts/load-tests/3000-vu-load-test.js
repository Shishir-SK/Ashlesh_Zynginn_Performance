// 3000-VU NORMAL LOAD TEST - ALL APIs
// Massive scale performance test
...
export let options = {
  stages: [
    ...
    { duration: '8m', target: 3000 },  // Sustained load
    { duration: '2m', target: 0 },    // Cool down
  ],
  discardResponseBodies: false,
  thresholds: {
    'api_response_time': ['p(95)<3000', 'p(99)<5000'],
...
export default function () {
  group('3000-VU API Load Test', function () {
    ...
    const success = check(response, {
      ...
      [`${endpoint.name} response time OK`]: (r) => r.timings.duration < 3000,
    }, {
      name: endpoint.name,
      endpoint: endpoint.path
    });
    ...
    console.log(`3000-VU ${endpoint.name}: Status ${response.status}, Time ${response.timings.duration}ms`);
    
    // Smaller think time for high load
